    ...
    const fetchTempo = async () => {
        ...
        try {
            const response = await axios.get(
                `https://api.open-meteo.com/v1/forecast?latitude=${props.latitudine}&longitude=${props.longitudine}&hourly=temperature_2m,weather_code,wind_speed_10m`
            );
    ...
    return (
        <div className="mx-3 mb-3">
            <Row className="text-center text-white">
                <Col className="prova" xs={3}>
                    {results?.hourly?.time?.slice(0, 0 + 6).map((hour, i) => (
                        <SingleHour

                            ora={hour.slice(11, 16)}
                            codtempo={results.hourly.weather_code[i]}
                            gradi={results.hourly.temperature_2m[i]}
                            key={i}
                        />
                    ))}
                </Col>
